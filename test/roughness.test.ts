...
describe('mapRoughness', () => {
  ...
  it('pins endpoints by default at every roughness level', () => {
    expect(
      mapRoughness(0, undefined, '#000', 2, 1).rough.preserveVertices,
    ).toBe(true);
    expect(
      mapRoughness(1, undefined, '#000', 2, 1).rough.preserveVertices,
    ).toBe(true);
  });
  ...
  it('clamps out-of-range input', () => {
    expect(
      mapRoughness(5, undefined, '#000', 2, 1).rough.roughness,
    ).toBeCloseTo(3.5);
