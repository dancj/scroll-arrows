...
const PKG = JSON.stringify({ name: "scroll-arrows", version: "0.1.0" }, null, 2);
...
describe("releaseChangelogRun", () => {
  it("skips when HEAD is not a merge commit", async () => {
    ...
    const res = await releaseChangelogRun({ gh: async () => ({ stdout: "" }), exec, now, ...fs });
  ...
  it("tags, bumps files, and opens a sync PR on the happy path", async () => {
    ...
    const gh = async (...a) => {
      ...
      if (a[1] === "create") return { stdout: "https://github.com/o/r/pull/77" };
      return { stdout: "" };
    };
    ...
    expect(res).toMatchObject({ version: "0.2.0", tag: "v0.2.0", prNumber: 77 });
