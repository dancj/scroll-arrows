...
function makeExec(routes) {
  return async (cmd, args) => {
    ...
    for (const [pat, val] of routes) if (key.includes(pat)) return { stdout: val };
    return { stdout: "" };
  };
}

const STAGING_PRS = JSON.stringify([
  { number: 1, title: "feat: arrows", mergedAt: "2026-06-18T00:00:00Z", author: { login: "dan" } },
]);
...
describe("autoReleasePrRun", () => {
  ...
  it("creates a release PR (no tags -> version from package.json + feat bump)", async () => {
    ...
    const gh = async (...args) => {
      calls.push(args);
      if (args[0] === "pr" && args[1] === "list" && args.includes("staging") && args.includes("merged")) {
        return { stdout: STAGING_PRS };
      }
      if (args[0] === "pr" && args[1] === "list") return { stdout: "" }; // no existing PR
      if (args[0] === "pr" && args[1] === "create") return { stdout: "https://github.com/o/r/pull/42" };
      return { stdout: "" };
    };
    const res = await autoReleasePrRun({ gh, exec, now, readFile: readPkg });
    expect(res).toMatchObject({ action: "created", prNumber: 42, version: "0.2.0" });
  ...
  it("updates the existing release PR in place", async () => {
    const exec = makeExec([["rev-list --count", "3"], ["tag --list", ""]]);
    const edits = [];
    const gh = async (...args) => {
      if (args[1] === "list" && args.includes("merged")) return { stdout: STAGING_PRS };
      if (args[1] === "list") return { stdout: JSON.stringify([{ number: 9, body: "old", title: "Release" }]) };
      if (args[1] === "edit") { edits.push(args); return { stdout: "" }; }
      return { stdout: "" };
    };
    const res = await autoReleasePrRun({ gh, exec, now, readFile: readPkg });
    expect(res).toMatchObject({ action: "updated", prNumber: 9, version: "0.2.0" });
