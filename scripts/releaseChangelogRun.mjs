...
export async function releaseChangelogRun({
  ...
  if (newestTag) {
    ...
    currentVersion = JSON.parse(await readFile(path.join(rootDir, "package.json"), "utf8")).version;
  }
  ...
  await writeFile(changelogPath, injectChangelogEntry(existingChangelog, entry));
  ...
  const out = await gh(
    ...
    "--base", "staging",
    "--head", branch,
    "--title", `chore: sync release v${version} (changelog + version)`,
    "--body", prBody,
  );
